...
Firework* FireworkParticleGenerator::generateParticle() {
	...
	return new Firework(position, velocity, particleType.particle3.radius - 2, particleType.particle3.mass,
		&PxSphereGeometry(particleType.particle3.radius), particleType.particle3.generation, particleType.particle3.life, particleType.particle3.color);
}
...
list<Firework*> FireworkParticleGenerator::createChildren(Firework* parent) {
	...
	case 3:
		...
		color = particleTypes.particle2.color;
		break;
	case 2:
		particlesToCreate = 4;
		...
		color = particleTypes.particle1.color;
		break;
	case 1:
		particlesToCreate = 5;
		...
		color = particleTypes.particle0.color;
		break;
	case 0:
		particlesToCreate = 0;
		radius = 0;
	...
	for (int i = 0; i < particlesToCreate; i++) {
		...
		fireworks.push_back(new Firework(parent->getPosition(), velocity, radius, mass,
			&PxSphereGeometry(radius), parent->getGen() - 1, life, color));
	}
